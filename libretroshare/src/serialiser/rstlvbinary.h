...
public:
	...
	~RsTlvBinaryData() override; // as per rule of three
        
		uint32_t TlvSize() const override;
		void	 TlvClear() override; /*! Initialize fields to empty legal values ( "0", "", etc) */
		void	 TlvShallowClear() override; /*! Don't delete the binary data */
	...
	std::ostream &print(std::ostream &out, uint16_t indent) const override; /*! Error/Debug util function */
